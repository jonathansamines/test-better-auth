    ...
    >
      ...
      <form onSubmit={onSsoSignIn} className="space-y-4">
        <p className="text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
          {ssoFromQuery
            ? "You opened the SSO entry point. Enter an optional email hint for your IdP, then continue."
            : "Use the SAML provider configured for this app (same flow as /sign-in/sso)."}
        </p>
        <div>
          <label htmlFor="sso-hint" className={labelClass}>
            Email hint{" "}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="h-10 animate-pulse rounded-xl bg-zinc-100 dark:bg-zinc-800" />
        <div className="h-11 animate-pulse rounded-xl bg-violet-200/50 dark:bg-violet-900/30" />
      </div>
    </AuthShell>
  );
}
